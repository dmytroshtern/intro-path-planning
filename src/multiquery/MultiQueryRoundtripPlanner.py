...
class MultiQueryRoundtripPlanner(PlanerBase):
    ...
    def planPath(self, startList: List[List[Any]], goalList: List[List[Any]], config) -> dict[str, Any]:
        ...
        """
        doBenchmark = config.get("benchmark", False)
        solutions = {}
        ...
        if not doBenchmark:
            ...
            bestSolution, bestCost, bestGraph = solutions["optimizedRoadmapShortcutSolution"]
            self.graph = bestGraph
    ...
    def _addSolution(self, solutions, solution, graph, name):
        cost = gu.pathLength(graph, solution)
        solutions[name] = (solution, cost, graph)
